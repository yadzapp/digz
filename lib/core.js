  ...
  async runOnce() {
    ...
    if (!('connect' in state)) {
      ...
      if (this.options.pingOnly) {
        ...
        state.ip = (state.gameHost || this.options.host || this.options.address)
        state.port_game = (state.gamePort || this.options.port)
        state.port_query = this.options.port
        state.ping = this.shortestRTT;
        delete state.gameHost;
        delete state.gamePort;
      }
      ...
      // Check if mods is present
      // if not, run fallback DZSA option for mods

      // Checking for specific responses
      // when data is fuzzy, this character '�' usually appears
      // so it's kind of safe to assume that the response is bugged
      function checkChar(obj) {
        let bool = []
        Object.values(obj).forEach((item) => {
          Object.values(item).forEach((insideItem) => {
            if (typeof insideItem === "string") {
              bool.push(insideItem.includes('�'))
            }
          })
        })
      ...
      if ((!('mods' in state) || checkChar(state.mods)) && !this.options.pingOnly) {
        // console.log('Running mods fallback option.');
        ...
        // XML Http Request
        // using [xmlhttprequest] dependency
        ...
        if (status >= 200 && status < 400) {
          // console.log('Request completed successfully - status:', status);
          
          // clean string and transform to object
          // and then reasign to state
          const modsString = response.responseText
          const modsStringUpdatedWorkshop = modsString.replace(/steamWorkshopId/g, 'id');
          const modsStringUpdatedName = modsStringUpdatedWorkshop.replace(/name/g, 'title');
          const modsObject = JSON.parse(modsStringUpdatedName)
