  ...
  async runOnce() {
    ...
    if (!('connect' in state)) {
      ...
      if (!('mods' in state)) {
        ...
        console.log('Mod fallback option');
        // console.log('it has no entry state.mods');
        
        // const fallback = await this.fallback.getMods(state)
        // console.log('fallback1:', fallback);
        
        // return state
        // let stateMods = []
        // console.log('promise?', typeof (this.fallback.getMods(state)));
        // return stateMods = await this.fallback.getMods(state)
        ...
        async function sendRequest() {
          // console.log('sendResquest fired');
        ...
        if (status === 0 || (status >= 200 && status < 400)) {
          ...
          // console.log('The request has been completed successfully - status', status);
          // console.log(response.responseText);
          const modsString = response.responseText
          // console.log('modsString', typeof (modsString));
          const modsStringUpdated = modsString.replace(/steamWorkshopId/g, 'id');
          // console.log('modsStringUpdated', modsStringUpdated);
          const modsObject = JSON.parse(modsStringUpdated)
      ...
      // console.log('it has no entry state.connect');
      return state
    }
    
    // console.log('it has entry state.connect');
    return state
