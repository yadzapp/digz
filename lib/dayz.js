...
class Valve extends Core {
  ...
  async cleanup(/** Results */ state) {
    ...
    // for debugging
    // state.mods = state.raw.dayzMods
    // console.log('typeof: ', typeof (state.mods));
    
    for (let i = 0; i <= state.raw.dayzMods.length - 1; i++) {
      // console.log('check inside if', state.mods[i]);
      // if (!state.mods[i].hasOwnProperty('workshopId')){
      // console.log('check inside if', state.mods[i].workshopId);
      if (state.raw.dayzMods[i].workshopId === undefined || state.raw.dayzMods[i].title === 'Livonia DLC'){
      // if (state.raw.dayzMods[i].workshopId === undefined){
        delete state.raw.dayzMods[i]
        console.log('state.raw.dayzMods[i]: ', state.raw.dayzMods[i]);
        // state.raw.dayzMods.splice(i, 1)
        // console.log('state.mods[i]: ', state.mods[i]);
        // console.log('state.raw.dayzMods[i].workshopId: ', state.raw.dayzMods[i].workshopId);
        // console.log('typeof: ', typeof (state.raw.dayzMods[i]));
        // console.log('length: ', state.raw.dayzMods[i].length);
      }
      
      
      // if (state.mods[i].workshopId = 294835219) {
      //   delete state.mods[i]
      // }
    }
    console.log('state.raw.dayzMods:', state.raw.dayzMods);
    ...
    // state.mods = JSON.parse(JSON.stringify(state.raw.dayzMods))
    // state.mods = filteredObj(state.raw.dayzMods)
    
    // Object.keys(state.raw.dayzMods).forEach(key => {
    //   if (state.raw.dayzMods[key] === '') {
    //     delete state.raw.dayzMods[key];
    //   }
    // });
    ...
    // Object.keys(state.mods).forEach(key => {
    //   if (state.mods[key] === null) {
    //     console.log('hey');
